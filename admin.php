...
function admin_screen_picker(string $prefix, array $options, array $checked, array $cfg = []): void
{
    ...
    foreach ($options as $opt) {
        ...
        $owner = (string)($blocked[$k] ?? '');
        $isBlocked = $owner !== '' && !$isChecked;
        $label = (string)$opt['name'];
        if ($isBlocked) {
            $label .= ' (' . $owner . ')';
        }
        echo '<label data-screen-key="' . h($k) . '"' . ($isBlocked ? ' class="screen-assign-taken"' : '') . '>'
            . '<input type="checkbox" name="' . h($checkboxName) . '" value="' . h($k) . '"'
            . ($isChecked ? ' checked' : '')
            . ($isBlocked ? ' disabled' : '')
            . '> ' . h($label) . '</label>';
    }
...
function admin_rotation_kiosk_settings_panel(
    ...
    array $rawConf,
    string $board,
    array $rssTickerFeeds,
    array $heroStripSources,
    array $heroStripKeyOptions,
      ...
      <div class="field-grid rotation-options-grid">
        ...
        <div class="field">
          <label class="check" title="Randomize play order once per full pass; every page appears once per cycle"><input type="checkbox" name="SCREEN_OPTS[<?= h($screenKey) ?>][shuffle]" value="1"
            <?= !empty($screenSettings['shuffle']) ? 'checked' : '' ?>> Shuffle each cycle</label>
          <div class="help" style="margin-top:4px">Random order, but every page plays once before the deck reshuffles.</div>
        </div>
        <div class="field span-2 rotation-section">
          <span class="mini">Bottom bar (weather ticker)</span>
        </div>
        <div class="field">
          <label class="check"><input type="checkbox" name="SCREEN_OPTS[<?= h($screenKey) ?>][show_ticker]" value="1"
            <?= $screenSettings['show_ticker'] ? 'checked' : '' ?>> Show weather alert ticker</label>
          <div class="help" style="margin-top:4px">NWS alerts scroll across the bottom of every page in rotation.</div>
        </div>
        <div class="field span-2" style="padding-top:0;margin-top:-4px">
          <label class="mini">RSS headlines when there are no weather alerts</label>
          <select name="SCREEN_OPTS[<?= h($screenKey) ?>][ticker_news_feed]">
            <option value="">Off — ticker hidden when no alerts</option>
          ...
          <div class="help" style="margin-top:6px">Optional fallback from a feed you manage under <strong>RSS Stories</strong>. Weather alerts always take priority.</div>
        </div>
        <div class="field span-2 rotation-section">
          <span class="mini">Screen schedule &amp; transitions</span>
        </div>
        <div class="field">
          ...
          <div class="help" style="margin-top:4px">Shows which page is loading on the kiosk (for troubleshooting).</div>
        </div>
        <div class="field">
          <label class="check"><input type="checkbox" name="SCREEN_OPTS[<?= h($screenKey) ?>][schedule_enabled]" value="1"
            <?= !empty($schedOpt['enabled']) ? 'checked' : '' ?>> Blank overnight</label>
          <div class="help" style="margin-top:4px">Black screen during off hours (see hours below).</div>
        </div>
        ...
        <div class="field span-2 rotation-section">
          <span class="mini">Hero status bar</span>
          <div class="help" style="margin:6px 0 10px">Persistent strip above the ticker — Kuma/Zabbix summaries, announcements, ntfy, etc.</div>
        </div>
        <div class="field span-2">
          <label class="check"><input type="checkbox" name="SCREEN_OPTS[<?= h($screenKey) ?>][hero_strip]" value="1"
            <?= !empty($heroCfg['enabled']) ? 'checked' : '' ?>> Enable hero status bar</label>
        </div>
        <div class="field span-2 rotation-section" style="padding-top:0;border-top:0;margin-top:0">
          ...
          <div class="field strip-height-field">
            <label class="mini">Bar height (px)</label>
        ...
        <div class="field span-2 rotation-section">
          <span class="mini">Location override</span>
          <div class="help" style="margin:6px 0 10px">Optional — overrides weather, air, UV, photo, traffic map, and alert ticker for this TV. Blank = use global Weather board (<?= h($globalLoc['place']) ?>).</div>
        ...
        <div class="field span-2 rotation-section">
          <span class="mini">Sports board override</span>
          <div class="help" style="margin:6px 0 10px">Teams shown on <code>sports.php</code> for this display (up to four). Blank = site default.</div>
...
<div class="wrap">
  ...
  <main<?= (!$tools && !$auditBoard && in_array($board, ['slides', 'rotation', 'status'], true)) ? ' class="main-wide"' : '' ?><?= ($auditBoard) ? ' class="main-wide"' : '' ?><?= (!$tools && !$auditBoard && $board === 'rotation') ? ' admin-board-rotation' : '' ?><?= (!$tools && !$auditBoard && $board === 'rotator') ? ' admin-board-rotator' : '' ?>>
    ...
    <?php elseif ($accountBoard): ?>
      ...
      ?>
      <div class="help" style="margin-bottom:18px">You manage
        <?= count($allowed) === 1 ? 'one display' : count($allowed) . ' displays' ?>:
        <?php if ($allowed === []): ?>
        ...
        ?>
          ...
          <div class="rotation-global-add" id="rotationGlobalAdd">
            <div class="rotation-global-add-row">
              ...
              <div>
                <label class="mini" for="rotationTargetScreen">Which display</label>
                ...
                <select id="rotationTargetScreen">
                  ...
                  ?>
                  <option value="<?= h($did) ?>" data-screen-key="<?= h((string)$sk) ?>"<?= $sk === $rotationDefaultScreenKey ? ' selected' : '' ?>><?= h(rotation_screen_display_name((string)$sk, $rotationScreens)) ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
              <?php else:
              ...
              <div>
                ...
                <select id="rotationTargetScreen" hidden aria-hidden="true">
                  <option value="<?= h($onlyDid) ?>" data-screen-key="<?= h($onlySk) ?>" selected><?= h($onlySk) ?></option>
                </select>
              </div>
              <?php endif; ?>
              <button type="button" class="addrow" onclick="addRotationPage(document.getElementById('rotationTargetScreen').value)" title="Add a page with a custom URL">+ Custom URL</button>
...
<?php if ((string)$screenKey !== (string)$rotationDefaultScreenKey): ?>
            <p class="help" data-playlist-placeholder style="margin:0">Open this playlist to load it (keeps the admin page small).</p>
          <?php else:
            require __DIR__ . '/admin_partials/rotation_playlist_body.php';
          endif; ?>
            </div>
                ...
                <div class="creator-editor">
                  <div class="field-block span-full">
                    ...
                    <div class="bg-section">
                      ...
                      <p class="field-hint" style="margin:0 0 8px">Five wall palettes (A–Z): Ember, Forest, Frost, GVSU, Lake Night (default). Photo scenes below are separate.</p>
...
function userScreensCellHtml(idx, role, checkedKeys, userId) {
  ...
  if (window.OPERATOR_MULTI_SCREEN) {
    ...
    let html = '<label class="l">Displays</label><div class="screen-picker screen-picker-compact screen-picker-assign" data-screen-picker data-summary-mode="assign">';
    html += '<div class="screen-picker-list slide-screen-checks">';
    opts.forEach(function (o) {
...
function initOperatorMultiScreenToggle() {
  ...
  toggle.addEventListener('change', function () {
    ...
    document.querySelectorAll('.user-card').forEach(function (card) {
      ...
      if (role === 'operator') refreshUserDisplayCell(card);
    });
  });
}
...
function bindLoadedRotationPlaylist(panel) {
  if (!panel) return;
  const deck = panel.querySelector('.rotation-playlist');
  if (deck) {
...
function loadRotationPlaylistPanel(screenKey, opts) {
  opts = opts || {};
  const panel = document.querySelector('.rotation-playlist-panel[data-rotation-screen="' + CSS.escape(screenKey) + '"]');
  if (!panel) return Promise.resolve(null);
  if (panel.getAttribute('data-playlist-lazy') !== '1') {
    if (opts.open) panel.setAttribute('open', '');
    return Promise.resolve(panel);
  }
  if (panel.dataset.playlistLoading === '1') return Promise.resolve(panel);
  panel.dataset.playlistLoading = '1';
  const body = panel.querySelector('.panel-body');
  if (body) body.innerHTML = '<p class="help" style="margin:0">Loading playlist…</p>';
  return fetch('admin.php?board=rotation&action=playlist_panel&screen=' + encodeURIComponent(screenKey), {
    credentials: 'same-origin',
    cache: 'no-store',
  })
    .then(function (r) { return r.ok ? r.text() : Promise.reject(new Error('HTTP ' + r.status)); })
    .then(function (html) {
      ...
      panel.removeAttribute('data-playlist-lazy');
      panel.dataset.playlistLoading = '';
      bindLoadedRotationPlaylist(panel);
      if (opts.open) panel.setAttribute('open', '');
      refreshBoardPickerInPlaylistMarks();
      return panel;
    })
    .catch(function () {
      panel.dataset.playlistLoading = '';
      if (body) {
        body.innerHTML = '<p class="help" style="margin:0;color:var(--bad)">Could not load playlist. Try refreshing.</p>';
      }
      return panel;
    });
}

function initRotationPlaylistLazyLoad() {
  document.querySelectorAll('.rotation-playlist-panel[data-playlist-lazy]').forEach(function (panel) {
    if (panel.dataset.playlistLazyBound) return;
    panel.dataset.playlistLazyBound = '1';
    panel.addEventListener('toggle', function () {
      if (!panel.open) return;
      const sk = panel.getAttribute('data-rotation-screen') || '';
      if (sk) loadRotationPlaylistPanel(sk, { open: true });
    });
  });
}
...
function initRotationGlobalAdd() {
  ...
  function syncCopyBtn() {
    if (copyBtn) copyBtn.style.display = sel.value === 'rotationDeck-main' ? 'none' : '';
  }
  function syncKioskSettingsPanel() {
    const opt = sel.options[sel.selectedIndex];
    const sk = opt ? (opt.getAttribute('data-screen-key') || '') : '';
    ...
    if (sk) {
      loadRotationKioskPanel(sk);
      loadRotationPlaylistPanel(sk, { open: true });
    }
  }
...
function setRotationPlaylistsOpen(open) {
  const panels = Array.from(document.querySelectorAll('.rotation-playlist-panel'));
  if (!open) {
    panels.forEach(function (el) { el.removeAttribute('open'); });
    return;
  }
  const lazyCount = panels.filter(function (el) {
    return el.getAttribute('data-playlist-lazy') === '1';
  }).length;
  if (lazyCount > 2 && !confirm('Load all ' + panels.length + ' playlists into this page? That can make Firefox slow again on large sites.')) {
    return;
  }
  panels.forEach(function (el) {
    const sk = el.getAttribute('data-rotation-screen') || '';
    if (!sk) {
  ...
  function render(data) {
    ...
    data.screens.forEach(function (s) {
      ...
      let ipHtml = '<div class="presence-stats">LAN: '
        ...
        + '</div>';
      if (s.client_ip) {
        ipHtml += '<div class="presence-stats">seen as <code>' + esc(s.client_ip) + '</code></div>';
        if (s.ip_via_proxy && s.remote_addr) {
          ipHtml += '<div class="presence-stats">via ' + esc(s.remote_addr) + '</div>';
        }
      } else if (s.remote_addr) {
        ipHtml += '<div class="presence-stats"><code>' + esc(s.remote_addr) + '</code></div>';
      }
