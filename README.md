...
**Display settings (super admin):** open the **Display settings** panel on the Rotation page to configure per-screen options in one table — weather ticker, clock, debug overlay, crossfade/settle/hang timings (blank = global default), **Weighted** / **Shuffle** rotation (see below), blank hours (CEC off/on), and HDMI-CEC. Kiosk URL per screen: `board.php?screen=KEY` (plain `board.php` = `main`). Operators see a smaller per-display options block inside their playlist panel.
...
**Weight & weighted rotation:** by default each playlist runs **sequentially** (or **shuffled** once per full pass). Enable **Weighted** on a display row in **Display settings** to switch that screen to weighted random: every time the rotation advances, the next page is chosen at random, and each entry's **Weight** controls how likely it is to be picked. Weight is an integer from **1** (normal, default) to **20**; higher numbers show more often — weight `3` is roughly three times as likely as weight `1`. Unlike shuffle, the same page can appear again before every other page has played, which is useful when you want weather or a headline board to dominate without removing RSS or photos entirely. **Weighted overrides Shuffle** if both are checked. Per-page weights are ignored when Weighted is off. Hour windows and **Skip** still apply in all modes. Set weights on each playlist card under **Weight** (expand a page or slide entry in the rotation editor).
