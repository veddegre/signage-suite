...
# Verify: curl -I http://HOST/config/settings.json  → 403
...
DirectoryIndex board.php index.php
...
# Older installs used /boards/ in the URL path — redirect to site root.
RedirectMatch permanent ^/boards/?(.*)$ /\$1
EOF
...
setup_nginx() {
  ...
  cat > "$snippet" <<EOF
# Home Signage Boards — generated by setup-server.sh
# Include inside your server { } block (root should point at WEBROOT).

location ^~ /config/ { deny all; return 403; }
location ^~ /cache/  { deny all; return 403; }
location ^~ /slides/ { deny all; return 403; }
location ^~ /photos/ { deny all; return 403; }
location ^~ /bin/    { deny all; return 403; }

# Older installs used /boards/ in the URL path.
location ^~ /boards/ {
    rewrite ^/boards/?(.*)$ /\$1 permanent;
}

location / {
    index board.php index.php;
...
url_path() {
  # Boards app is served at the vhost root (DocumentRoot = WEBROOT).
  echo ""
}
...
Security checklist:
  ...
  • Behind a reverse proxy: proxy → http://this-host/ ; kiosks use the proxy https URL
