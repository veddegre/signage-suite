...
{
    $d = cfg('video.VIDEO_DIR', SIGNAGE_ROOT . '/videos');
    if ($d === '' || $d === './videos' || $d === 'videos') {
        return SIGNAGE_ROOT . '/videos';
    }
    if ($d[0] !== '/') {
...
{
    return SIGNAGE_ROOT . '/bin/yt-dlp';
}
...
{
    $base = realpath(SIGNAGE_ROOT . '/config/cookies');
    if ($base === false) {
        $base = SIGNAGE_ROOT . '/config/cookies';
    }
    $dir = dirname($path);
